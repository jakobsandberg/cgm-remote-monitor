...
function init (client, d3) {
  ...
  function hasEventType(entry) {
    return entry.eventType && client.careportal.resolveEventName(entry.eventType) && client.careportal.resolveEventName(entry.eventType) !== 'None';
  }
  ...
  renderer.addTreatmentCircles = function addTreatmentCircles (nowDate) {
    function treatmentTooltip (d) {
      ...
      return '<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(d)) + '<br/>' +
        (hasEventType(d) ? '<strong>' + translate('Treatment type') + ':</strong> ' + translate(client.careportal.resolveEventName(d.eventType)) + '<br/>' : '') +
  ...
  function appendTreatments (treatment, arc) {
    ...
    function treatmentTooltip () {
      ...
      client.tooltip.style('opacity', .9);
      client.tooltip.html('<strong>' + translate('Time') + ':</strong> ' + client.formatTime(getOrAddDate(treatment)) + '<br/>' +
          (hasEventType(treatment) ? '<strong>' + translate('Treatment type') + ':</strong> ' + translate(client.careportal.resolveEventName(treatment.eventType)) + '<br/>' : '') +
